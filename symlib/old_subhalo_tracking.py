...
def main():
    ...
    for snap in [235]:
        ...
        #for h0 in [377]:
            ...
            xc, vc = h["x_core"][h0,snap], h["v_core"][h0,snap]
            #xc, vc = xcs[0], vcs[0]

            print(h["x_core"][h0,snap])
            print(h["x"][h0,snap])
            print(xcs[0])
            ...
            r95_bound = np.quantile(rc[ok][bound], 0.95)
            r99_bound = np.quantile(rc[ok][bound], 0.99)
            ...
            for i_ax in range(len(axs)):
                ...
                util.plot_circle(ax1, h["x"][h0,snap][0], h["x"][h0,snap][1],
                                 h["rvir"][h0,snap], c=pc("g"))
                util.plot_circle(ax2, h["x"][h0,snap][2], h["x"][h0,snap][1],
                                 h["rvir"][h0,snap], c=pc("g"))

                util.plot_circle(ax1, h["x_core"][h0,snap][0],
                                 h["x_core"][h0,snap][1],
                                 r95_bound, c=pc("p"))
                util.plot_circle(ax2, h["x_core"][h0,snap][2],
                                 h["x_core"][h0,snap][1],
                                 r95_bound, c=pc("p"))
                ...
                ax1.set_xlabel(r"$X\,(h^{-1}{\rm kpc})$")
                ax1.set_ylabel(r"$Y\,(h^{-1}{\rm kpc})$")
            ...
            for mi in range(len(methods)):
                ...
                if methods[mi] == "circular":
                    ...
                    ax.plot(r_cut[ok], r_tidal_bound_i[ok], ":",
                            c=pc("b", 0.75))
                if methods == "centripetal":
                    i_min = np.argmin(np.abs(r_tidal_bound - r_cut))
                    r_tidal = r_tidal_bound[i_min]
                    util.plot_circle(ax1, h["x_core"][h0,snap][0],
                                     h["x_core"][h0,snap][1],
                                     r_tidal, c=pc("b"))
                    util.plot_circle(ax2, h["x_core"][h0,snap][2],
            ...
            plt.fill_between([0, r_high], [r99_bound, r95_bound],
                             color="k", alpha=0.2)
