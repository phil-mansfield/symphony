...
def main():
    ...
    for i_suite in range(n_suite):
        ...
        suite_name = suite_names[i_suite]
        base_dir = path.join(base, suite_name)
        sub_mvir, sub_mpeak, sub_minfall, n_host = [], [], [], 0

        # loop over all hosts
        for halo_name in sorted(os.listdir(base_dir)):
            if len(halo_name) < 4 or halo_name[:4] != "Halo": continue

            # Read subhalo information
            sim_dir = path.join(base_dir, halo_name)
            param = symlib.parameter_table[suite_name]
            halos, hists = symlib.read_subhalos(param, sim_dir)
